...
def train(model, data, optimizer, loss_fn, epoch):
    ...
    for batch_idx, batch in enumerate(data):
        ...
        input_ids1 = jt.Var(np.array(input_ids1))
        attention_mask1 = jt.Var(np.array(attention_mask1))
        input_ids2 = jt.Var(np.array(input_ids2))
        attention_mask2 = jt.Var(np.array(attention_mask2))
        ...
        outputs2 = model(input_ids2, attention_mask=attention_mask2)
         ...
         # bce loss
        probs1 = jt.nn.softmax(outputs1.logits, dim=1).squeeze()
        probs2 = jt.nn.softmax(outputs2.logits, dim=1).squeeze()
...
train_small = train_batches[0:4]
valid_small = valid_batches[0:4]
