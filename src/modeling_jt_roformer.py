...
def load_tf_weights_in_roformer(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        pointer.data = jt.from_numpy(array)
    return model
    ...
    def forward(self, input_ids=None, token_type_ids=None, inputs_embeds=None):
        ...
        if token_type_ids is None:
            token_type_ids = jt.zeros(
                input_shape, dtype=jt.Var.int64, device=inputs_embeds.device
            )
...
class RoFormerSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = jt.cat([past_key_value[0], key_layer], dim=2)
            value_layer = jt.cat([past_key_value[1], value_layer], dim=2)
        else:
            key_layer = self.transpose_for_scores(self.key(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
            # rotary key_layer & value_layer
    ...
    def apply_rotary(x, sinusoidal_pos):
        ...
        return jt.stack([x1 * cos - x2 * sin, x2 * cos + x1 * sin], dim=-1).flatten(-2, -1)
...
class RoFormerLMPredictionHead(nn.Module):
    def __init__(self, config):
        ...
        self.bias = nn.Parameter(jt.zeros(config.vocab_size))
